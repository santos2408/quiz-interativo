...
const createElementWithTestId = (element, testId) => {
  const newElement = document.createElement(element);
  newElement.setAttribute('data-testid', testId);
  return newElement;
};

const popupTitleElement = createElementWithTestId('h2', 'popup-title');
const popupScoreElement = createElementWithTestId('p', 'popup-score');
...
const closePopup = (event) => {
  ...
  const classesThatClosePopup = [
    'popup-close',
    'popup-button',
    'popup-wrapper'
  ];
  const clickedElementHasClass = classesThatClosePopup.includes(
    clickedElement.classList[0]
  );
